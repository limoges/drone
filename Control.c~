...
//#define k01	(1.5)		//	Gain k1	->	Altitude
//#define k02	(0.05)		//	Gain k2	->	Altitude
//#define ka0	(k01*k02+1.0)	//	Gain ka	->	Altitude
//#define kb0	(k01+k02)		//	Gain kb	->	Altitude
#define ka0	(AAA)		//	Gain ka	->	Altitude
#define kb0	(BBB)		//	Gain kb	->	Altitude

//#define k11	(1.0)		//	Gain k1	->	Roulis
//#define k12	(0.05)		//	Gain k2	->	Roulis
//#define ka1	(k11*k12+1.0)	//	Gain ka	->	Roulis
//#define kb1	(k11+k12)		//	Gain kb	->	Roulis
#define ka1	(AAA)		//	Gain ka	->	Roulis
#define kb1	(BBB)		//	Gain kb	->	Roulis

//#define k21	(1.0)		//	Gain k1	->	Tangage
//#define k22	(0.05)		//	Gain k2	->	Tangage
//#define ka2	(k21*k22+1.0)	//	Gain ka	->	Tangage
//#define kb2	(k21+k22)
...
//#define k31	(1.0)		//	Gain k1	->	Lacet
//#define k32	(0.05)
//	Gain k2	->	Lacet
//#define ka3	(k31*k32+1.0)	//	Gain ka	->	Lacet
//#define kb3	(k31+k32)		//	Gain kb	->	Lacet
...
//		DotError[YAW]    = SpeedD.Yaw - SpeedM.Yaw;
		DotError[YAW]    = DataD.Yaw - SpeedM.Yaw;

//printf("Error : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",Error[HEIGHT],Error[ROLL],Error[PITCH],Error[YAW]);
//printf("dotError : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",DotError[HEIGHT],DotError[ROLL],DotError[PITCH],DotError[YAW]);
//printf("Error : %10.5lf  %10.5lf\n",Error[ROLL],Error[PITCH]);
//printf("dotError : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",DotError[HEIGHT],DotError[ROLL],DotError[PITCH],DotError[YAW]);
//printf("Angle : %10.5lf  %10.5lf  %10.5lf\n",DataM.Roll,DataM.Pitch,DataM.Yaw);

//		U[0] = (m/(cos(DataM.Roll)*cos(DataM.Pitch)))*(g + ka0*Error[HEIGHT] + kb0*DotError[HEIGHT]);
		U[0] = DataD.Elevation;
		Wr   = -W[0] + W[1] - W[2] + W[3];
...
//		U[3] = (1.0/b3)*(-a5*SpeedM.Roll*SpeedM.Pitch + ka3*Error[YAW] + kb3*DotError[YAW]);

//		U[3] = 0.3*DotError[YAW];
...
//		PWM[0] = (PWM[0] < DEADZONE) ? 0.0 : PWM[0];
//		PWM[1] = (PWM[1] < DEADZONE) ? 0.0 : PWM[1];
//		PWM[2] = (PWM[2] < DEADZONE) ? 0.0 : PWM[2];
//		PWM[3] = (PWM[3] < DEADZONE) ? 0.0 : PWM[3];
...
//		PWM[0] = (PWM[0] > 1.0) ? 1.0 : PWM[0];
//		PWM[1] = (PWM[1] > 1.0) ? 1.0 : PWM[1];
//		PWM[2] = (PWM[2] > 1.0) ? 1.0 : PWM[2];
//		PWM[3] = (PWM[3] > 1.0) ? 1.0 : PWM[3];
...
//printf("U   : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",U[0],U[1],U[2],U[3]);
//printf("W   : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",W[0],W[1],W[2],W[3]);
//printf("PWM : %10.5lf  %10.5lf  %10.5lf  %10.5lf\n",PWM[0],PWM[1],PWM[2],PWM[3]);

		pthread_spin_lock(&(Motor->MotorLock));
//	   	Motor->pwm[0] = (uint16_t)(0);
//	   	Motor->pwm[1] = (uint16_t)(0);
//	   	Motor->pwm[2] = (uint16_t)(0);
//	   	Motor->pwm[3] = (uint16_t)(0);
...
int ControlInit (ControlStruct *Control) {
	...
	param.sched_priority = minprio + 3*(maxprio - minprio)/4;
