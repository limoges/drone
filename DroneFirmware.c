...
void SigTimerHandler (int signo) {
	...
	if ((MotorActivated) && ((Period % MOTOR_PERIOD) == 0))
	...
	if ((ControlActivated) && ((Period % CONTROL_PERIOD) == 0))
...
//		printf("%s rlim_cur = %lu   rlim_max = %lu  !!!\n", __FUNCTION__, (unsigned long) rlim.rlim_cur, (unsigned long) rlim.rlim_max);
//	}
