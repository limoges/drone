...
SensorParam ParamData[NUM_SENSOR] = { ACCEL_PARAM, GYRO_PARAM, SONAR_PARAM,
		BAROM_PARAM, MAGNETO_PARAM };
SensorStruct SensorTab[NUM_SENSOR] = { ACCEL_INIT, GYRO_INIT, SONAR_INIT,
		BAROM_INIT, MAGNETO_INIT };
AttitudeStruct AttitudeTab[NUM_SENSOR] = { ACCEL_ATT_INIT, GYRO_ATT_INIT,
		SONAR_ATT_INIT, BAROM_ATT_INIT, MAGNETO_ATT_INIT };
...
int main(int argc, char *argv[]) {
	...
	int i, j, retval = 0;
	int ch = 0;
	//TODO: REMOVE DEBUG VARIABLES BELOW
	...
	printf(
			"	   Option :	Pas d'option -> Par défaut (Pas de Log, IP = 192.168.1.2)\n");
	...
	printf(
			"	Note : Les options peuvent être cumulées et dans n'importe quel ordre.\n");
	...
	while (ch != 'q') {
		...
		case '4':
			...
			break;
		case 'o':
			Motor.led[index] = MOTOR_LEDORANGE;
			break;
		case 'r':
			Motor.led[index] = MOTOR_LEDRED;
			break;
		case 'g':
			Motor.led[index] = MOTOR_LEDGREEN;
			break;
		case 'b':
			Motor.led[index] = MOTOR_LEDOFF;
			break;
		case 'a':
		...
		Motor.pwm[0] = motor_speed;
		Motor.pwm[1] = motor_speed;
		Motor.pwm[2] = motor_speed;
		Motor.pwm[3] = motor_speed;
	}
